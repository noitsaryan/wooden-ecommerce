...
const SearchProduct = ({searchItem, response, position}) => {
  return (
    <div className={`w-96 translate-x-52 md:translate-x-0 ${position} rounded max-h-80 overflow-y-scroll-scroll gap-3 flex-col bg-slate-50 fixed top-16 z-20 p-2 ${searchItem ? "flex" : "hidden"}`}>
   {
    response.length>0 ? response && response.map((elem ,i)=>(
      ...
      <Link href={`/shop/category/${elem.sku}`}><Button size='sm' className='bg-Primary'>View Product</Button></Link>
